...
{
    const HEADER_CODE = 'Api-Code';
    const HEADER_MSG = 'Api-Message';
    const STATUS_HTTP = [
        100 => 'Continue',
    ...
    private int $idHttpRequest;

    public function setIdHttpRequest(int $id): static
    {
        $this->idHttpRequest = $id;
        return $this;
    }

    public function getIdHttpRequest(): int
    {
        return $this->idHttpRequest;
    }


    public function getOptionsResponse(): array
    {
        return [
            'status' => $this->code(),
    ...
    public function setDataDecoded(array|null $data = null): static
    {
    ...
    {
        $code = $this->header(self::HEADER_CODE);
    ...
    public function message(): string
    {
        ...
        $message = $this->header(self::HEADER_MSG);
        $message = empty($message) ? $this->toPsrResponse()->getReasonPhrase() : $message;
        $message = empty($message) ? (self::STATUS_HTTP[$this->code()] ?? $default) : $message;
        return $message;
    }
    ...
    public function error(Closure $error): static
    {
        if (empty($this->ok())) {
            if ($error instanceof Closure) {
                ...
                return ((($return instanceof static) || (is_subclass_of($return, static::class))) ? $return : $this);
            }
        }
        return $this;
    }
    ...
    public function current(array $data = []): mixed
    {
        $first = current($data);
        if (is_array($first)) {
